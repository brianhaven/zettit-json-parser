#!/usr/bin/env python3
"""
Test Script for Git Issue #21 - Testing v4 Extractor (No V2 Fallback)
=======================================================================

Tests Script 03 v4 (PureDictionaryReportTypeExtractor) with Issue #21 sample titles.
This version completely removes V2 fallback and uses dictionary-only processing.
...
def create_output_directory() -> str:
    ...
    output_dir = os.path.join(outputs_dir, f"{timestamp}_issue_21_v4_test_results")
...
def main():
    """Test Script 03 v4 with sample titles for Issue #21."""
    print("\n" + "="*80)
    print("TESTING SCRIPT 03 V4 - GIT ISSUE #21 SAMPLE TITLES (NO V2 FALLBACK)")
    print("="*80)
    ...
    try:
        ...
        print("✓ Successfully imported required modules (v4 extractor)")
    ...
    with open(results_file, 'w') as f:
        f.write("# Git Issue #21 - Script 03 v4 Test Results (No V2 Fallback)\n\n")
        ...
        f.write(f"**Extractor:** Script 03 v4 (PureDictionaryReportTypeExtractor)\n")
        f.write(f"**V2 Fallback:** REMOVED (Dictionary-only processing)\n\n")
            ...
            try:
                ...
                if not test_success:
                    if not report_type_match:
                        print(f"         Expected Report: '{expected_report_type}'")
                ...
                f.write(f"**Keywords Detected:** {result.keywords_detected}\n")
    ...
    print(f"\n" + "="*80)
    print("TEST SUMMARY - SCRIPT 03 V4 (NO V2 FALLBACK)")
    print("="*80)
    ...
    if success_count == total_tests:
        ...
        improvement = success_rate - 60.0  # Baseline was 60%
        if improvement > 0:
            print(f"📈 IMPROVEMENT: +{improvement:.1f}% vs baseline (60.0%)")
        else:
            print(f"📉 REGRESSION: {improvement:.1f}% vs baseline (60.0%)")
    ...
    with open(results_file, 'a') as f:
        ...
        if success_count == total_tests:
            ...
            improvement = success_rate - 60.0  # Baseline was 60%
            if improvement > 0:
                f.write(f"📈 **IMPROVEMENT:** +{improvement:.1f}% vs baseline (60.0%)\n")
            else:
                f.write(f"📉 **REGRESSION:** {improvement:.1f}% vs baseline (60.0%)\n")
